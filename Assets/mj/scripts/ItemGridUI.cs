...
public class ItemGridUI : MonoBehaviour
{
    ...
    private void RefreshExistingItemVisuals()
    {
        foreach (InventoryItem item in gridManager.GetItems())
        {
            HandleItemPlaced(item, item.OriginPosition);
        }
    }
    ...
    public bool TryAddItem(ItemData itemData, int quantity = 1)
    {
        ...
        if (!inventoryInitialized)
        {
            storedItems.Add(new InitialItemEntry { itemData = itemData, quantity = quantity });
            return true;
        }

        InventoryItem item = new InventoryItem(itemData, quantity);
        if (!gridManager.FindSpaceForItem(item, out Vector2Int position))
            return false;

        return gridManager.PlaceItem(item, position.x, position.y);
    }
