...
public class BaseSplash : MonoBehaviour, IProjectileType
{
    [Header("Collision")]
    ...
    private float splashRadius = 2.0f; // Define the radius of the splash effect
    private bool hit = false; // Flag to indicate if the splash has hit something
    ...
    private void Splash()
    {
        Collider2D[] hitColliders = Physics2D.OverlapCircleAll(transform.position, splashRadius, hittableLayers);

        foreach (Collider2D hit in hitColliders)
        {

            var health = hit.GetComponent<PlayerStats>();

            if (health != null)
            {
                Vector2 hitPos = hit.transform.position;
                float distance = Vector2.Distance(transform.position, hitPos);

                float distanceRatio = Mathf.Clamp01(distance / splashRadius);
                float damageMultiplier = 1.0f - distanceRatio;
