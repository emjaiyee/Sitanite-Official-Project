...
[AddComponentMenu("Dimla/Gateway Room Visibility")]
public class GatewayRoomVisibility : GatewayVisibility
{
    [Serializable]
    ...
    [Header("Room Gateway Path")]
    [Tooltip("Each tilemap has its own visibility state while the room gateway is unlocked.")]
    [SerializeField] private List<RelatedPathTilemap> relatedTilemaps =
        new List<RelatedPathTilemap>();

    [Tooltip("Each sprite has its own visibility state while the room gateway is unlocked.")]
    [SerializeField] private List<RelatedPathSprite> relatedSprites =
        new List<RelatedPathSprite>();

    [Tooltip("Each collider has its own enabled state while the room gateway is unlocked.")]
    ...
    public void UnlockRoomProgression()
    {
        SetVisible(true);
    }
