...
[AddComponentMenu("Dimla/Gateway Secret Visibility")]
public class GatewaySecretVisibility : GatewayVisibility
{
    [Serializable]
    ...
    [Header("Secret Room Path")]
    [Tooltip("Each tilemap has its own visibility state while the secret room is unlocked.")]
    [SerializeField] private List<RelatedPathTilemap> relatedTilemaps =
        new List<RelatedPathTilemap>();

    [Tooltip("Each sprite has its own visibility state while the secret room is unlocked.")]
    [SerializeField] private List<RelatedPathSprite> relatedSprites =
        new List<RelatedPathSprite>();

    [Tooltip("Each collider has its own enabled state while the secret room is unlocked.")]
